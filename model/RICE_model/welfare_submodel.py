...
#
#         # output metrics
#         self.discount_factors_utility = np.zeros((self.n_regions, self.simulation_horizon))
#
#     def utilitarian_metrics(self, t, year):
#
#
...
import numpy as np
import pandas as pd
from model.RICE_model.model_limits import ModelLimits
import os
...
class WelfareSubmodel:
    def __init__(self, years, regions):
        ...
        self.discount_factors_utility = np.zeros((self.n_regions, self.simulation_horizon))
        self.period_utilities = np.zeros((self.n_regions, self.simulation_horizon))
        self.utility_welfares = np.zeros((self.n_regions, self.simulation_horizon))
        self.inst_util_ww = np.zeros((self.n_regions, self.simulation_horizon))
        self.per_util_ww = np.zeros((self.n_regions, self.simulation_horizon))
        self.global_per_util_ww = np.zeros(self.simulation_horizon)
        self.global_damage = np.zeros(self.simulation_horizon)
    ...
    def run_utilitarian(self, t, year, CPC, labour_force, damages, net_output, temp_atm, irstp):
        def calculate_social_discount_factor(t, irstp):
            # social discount factor for utility
            self.discount_factors_utility[:, t] = 1 / (
                    (1 + irstp) ** (self.delta_t * t)
            )
            return self.discount_factors_utility

        def calculate_utility_of_consumption(t, CPC):
            # period utilities = utilities for eac region at each time
            # instantaneous welfare without welfare weights
            self.period_utilities[:, t] = (1 / (1 - self.elasmu)) * (CPC[:, t]) ** (
                    1 - self.elasmu
            ) + 1
            # Should it be -1 in the end because that's what the CRRA equation says -> this is a comment from Max Reddel, left it in to keep the model as consistent as possible
            ...
            # Instantaneous utility function with welfare weights
            self.inst_util_ww[:, t] = self.period_utilities[:, t] * self.Alpha_data[:, t]
            return self.inst_util_ww

        def calculate_welfare_based_on_consumption(t, labour_force):
            # period utility with welfare weights
            self.per_util_ww[:, t] = (
                    self.inst_util_ww[:, t]
                    * labour_force[:, t]
            ...
            return self.per_util_ww

        def calculate_global_metrics(t, net_output, damages):
            # Global period utility with welfare weights
            self.global_per_util_ww[t] = self.per_util_ww[:, t].sum(axis=0)
            ...
            # Global damages
            self.global_damage[t] = damages[:, t].sum(axis=0)
            return self.global_per_util_ww, self.global_output, self.global_damage

        self.discount_factors_utility = calculate_social_discount_factor(t, irstp)
        self.inst_util_ww = calculate_utility_of_consumption(t, CPC)
        self.per_util_ww = calculate_welfare_based_on_consumption(t, labour_force)
        self.global_per_util_ww, self.global_output, self.global_damage = calculate_global_metrics(t, net_output, damages)
